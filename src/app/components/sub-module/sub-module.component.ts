...
export class SubModuleComponent {
  ...
  resmap = new Map<string, number>(); 
          ...
          for(let ele of this.dataSource) { 
            ...
            if(this.barChartLabels.indexOf(ele.custom_submodule)==-1){
               this.barChartLabels.push(ele.custom_submodule); 
              } 
            } 
  ...
  populateDataSets(){
   for(let ele of this.dataSource) {  
          ...
          if(this.finalmap.has(ele.custom_submodule)){

          let statusmap =  this.finalmap.get(ele.custom_submodule);
          if(statusmap.has(STATUS[ele.status_id])){
            let ct = statusmap.get(STATUS[ele.status_id])
             statusmap.set(STATUS[ele.status_id],(ct+1))
              this.finalmap.set(ele.custom_submodule,statusmap);
          }else{
           ...
           this.finalmap.set(ele.custom_submodule,mymap);
          }  
     }
