...
import { STATUS} from '../dynamic-table/data'

@Component({
  selector: 'app-application-wise',
  templateUrl: './application-wise.component.html',
  styleUrls: ['./application-wise.component.scss']
})
export class ApplicationWiseComponent {


  @Input() dataSource:any = [];
  @Input() onlypassed:string;
  ...
  resmap = new Map<string, number>(); 
          ...
          for(let ele of this.dataSource) { 
            if(
            this.barChartLabels.indexOf(ele.custom_application)==-1){
               this.barChartLabels.push(ele.custom_application); } } 
  ...
  populateDataSets(){
   for(let ele of this.dataSource) {
          ...
          if(this.finalmap.has(ele.custom_application)){

          let statusmap =  this.finalmap.get(ele.custom_application);
          if(statusmap.has(STATUS[ele.status_id])){
            let ct = statusmap.get(STATUS[ele.status_id])
             statusmap.set(STATUS[ele.status_id],(ct+1))
              this.finalmap.set(ele.custom_application,statusmap);
          }else{
           ...
           this.finalmap.set(ele.custom_application,mymap);
          }
