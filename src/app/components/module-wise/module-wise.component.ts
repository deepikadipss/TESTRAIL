...
@Component({
  selector: 'app-module-wise',
  templateUrl: './module-wise.component.html',
  styleUrls: ['./module-wise.component.scss']
})
export class ModuleWiseComponent  {
  ...
  resmap = new Map<string, number>(); 
          ...
          for(let ele of this.dataSource) { 
            if(
            this.barChartLabels.indexOf(ele.custom_module)==-1){
               this.barChartLabels.push(ele.custom_module); } } 
  ...
  populateDataSets(){
   for(let ele of this.dataSource) {
          ...
          if(this.finalmap.has(ele.custom_module)){

          let statusmap =  this.finalmap.get(ele.custom_module);
          if(statusmap.has(STATUS[ele.status_id])){
            let ct = statusmap.get(STATUS[ele.status_id])
             statusmap.set(STATUS[ele.status_id],(ct+1))
              this.finalmap.set(ele.custom_module,statusmap);
          }else{
           ...
           this.finalmap.set(ele.custom_module,mymap);
          }
